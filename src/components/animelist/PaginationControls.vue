  ...
  <div v-if="totalPages > 1" class="mb-8 flex flex-col items-center gap-4 px-4">
    ...
    <div class="flex w-full max-w-md items-center justify-center gap-2">
      ...
      <div class="hidden items-center gap-3 px-4 sm:flex">
        ...
        <Select v-model:modelValue="localPage">
          <SelectTrigger class="min-w-[80px]">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectItem v-for="page in totalPages" :key="page" :value="page - 1">{{ page }}</SelectItem>
          </SelectContent>
        </Select>
        <span class="flex gap-1.5 text-sm font-medium text-muted-foreground">de {{ totalPages }}</span>
      </div>

      <!-- Indicador de página móvil central -->
      <div class="flex min-w-[60px] items-center justify-center rounded-lg border px-3 py-2 sm:hidden">
    ...
    <div class="flex w-full max-w-xs sm:hidden">
      <Select v-model:modelValue="localPage">
        ...
        <SelectContent>
          <SelectItem v-for="page in totalPages" :key="page" :value="page - 1">Página {{ page }}</SelectItem>
        </SelectContent>
      </Select>
    </div>
    ...
    <div class="w-full max-w-xs sm:max-w-sm">
      <Progress 
        :model-value="((currentPage + 1) / totalPages) * 100" 
        class="h-2"
      />
    </div>
  </div>
