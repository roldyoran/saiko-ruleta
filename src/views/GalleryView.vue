<template>
  <main>
    ...
    <section v-for="[temporada, usuarios] in Object.entries(data)" :key="temporada">
      <h1
        class="py-4 text-3xl bg-clip-text text-transparent bg-gradient-to-r from-[#e94fe1] to-[#f37018] font-extrabold flex items-center justify-center"
      >
        {{ temporada }}
      </h1>
          ...
          <template v-if="Array.isArray(contenido.imagenes)">
            ...
            >
              ...
              <a
                ...
                class="absolute bg-rose-950 bottom-2 left-2 z-10 px-3 py-1.5 rounded-md text-white font-semibold text-sm shadow transition-all duration-300 ease-in-out hover:shadow-lg hover:bg-gradient-to-br hover:from-purple-600 hover:via-indigo-500 hover:to-pink-500"
                @click.stop
              >
                @{{ usuario }}
              </a>
