...
function clearSafetyTimeout() {
  if (safetyTimeout.value && typeof window !== 'undefined') {
...
function startAutoplay() {
  stopAutoplay()
  if (!props.autoplay || !images.value.length || typeof window === 'undefined') return
  timerId.value = window.setInterval(() => { go(1) }, Math.max(100, props.interval))
}

function stopAutoplay() {
  if (timerId.value && typeof window !== 'undefined') {
...
.image-carousel {
  ...
  height: 500px;
  width: 190px;
  background: rgb(35, 35, 36); /* bg-gray-800 equivalent */
